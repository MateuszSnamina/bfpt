...
TEST(RangeStreamer, RawRangeStreamerSixElementRange) {
    ...
    ASSERT_EQ(extension::boost::RangeStringStreamer().stream(v1).str(), "{0:13,1:14,2:15,3:16,4:11,5:12}");
}
...
TEST(RangeStreamer, Fancy) {
    ...
    const auto s = extension::boost::RangeStringStreamer()
...
TEST(RangeStreamer, FormatIndependencyFlagOff) {
    ...
    oss1.fill('*');
    oss1 << std::setw(5);
    extension::boost::RangeStreamer(oss1)
        ...
        .stream(v1);
    oss1 << 1.4;
    ASSERT_EQ(oss1.str(), "****{*A:*+1.10|*B:*+1.20|*C:*+1.30}+1.40");
}
...
TEST(RangeStreamer, FormatIndependencyFlagExplicitOn) {
    ...
    extension::boost::RangeStreamer(oss1)
        ...
        .stream(v1);
    oss1 << 1.4;
    ASSERT_EQ(oss1.str(), "{ A: +1.10| B: +1.20| C: +1.30}**1.4");
}
