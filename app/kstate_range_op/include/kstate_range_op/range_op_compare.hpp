...
bool compare_less(const ForwardRange1& rng1, const ForwardRange2& rng2) {
    assert(boost::size(rng1) == boost::size(rng2));
    return boost::lexicographical_compare(rng1, rng2);
}

template <typename ForwardRange1, typename ForwardRange2>
bool compare_equality(const ForwardRange1& rng1, const ForwardRange2& rng2) {
    assert(boost::size(rng1) == boost::size(rng2));
    return boost::range::equal(rng1, rng2);
}

template <typename ForwardRange1, typename ForwardRange2>
std::optional<size_t> compare_translational_equality(const ForwardRange1& rng1, const ForwardRange2& rng2) {
    assert(boost::size(rng1) == boost::size(rng2));
