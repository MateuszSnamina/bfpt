...
make_kstate_streamer(
        KstateT&&,
        const extension::boost::RangeStreamerSettings<typename remove_cvref_t<KstateT>::SiteState>);
...
public: // Helper types:
    ...
    using SiteState = typename remove_cvref_t<KstateT>::SiteState;
public: // Factory function:
    friend
    KstateStreamer<KstateT>
    make_kstate_streamer<KstateT>(KstateT&&, const extension::boost::RangeStreamerSettings<SiteState>);
public: // API:
    std::ostream& stream(std::ostream& os) const;
    std::string str() const;
private:
    KstateStreamer(std::add_rvalue_reference_t<KstateT>, const extension::boost::RangeStreamerSettings<SiteState>);
private:
    const KstateT _kstate;
    const extension::boost::RangeStreamerSettings<typename remove_cvref_t<KstateT>::SiteState> _range_streamer_settings;
};
...
KstateStreamer<_KstateT>::KstateStreamer(
        ...
        const extension::boost::RangeStreamerSettings<SiteState> range_streamer_settings) :
...
KstateStreamer<_KstateT>::stream(std::ostream& os) const{
    ...
    const auto default_stream_value_putter = [](std::ostream& s, SiteState t) { s << t; };
...
KstateStreamer<KstateT> make_kstate_streamer(
        KstateT&& kstate,
        const extension::boost::RangeStreamerSettings<typename remove_cvref_t<KstateT>::SiteState> range_streamer_settings) {
