...
TEST(Basis, Empty) {
    ...
    const auto k100 = std::make_shared<kstate::DynamicKstate<kstate_trait::TraitSiteState<int>>>(v100, ctr_from_range);
...
TEST(Basis, OneElement) {
    ...
    const auto k10 = std::make_shared<kstate::DynamicKstate<kstate_trait::TraitSiteState<int>>>(v10, ctr_from_range);
...
TEST(Basis, TwoDifferentElementsTest0) {
    ...
    const auto k1 = std::make_shared<kstate::DynamicKstate<kstate_trait::TraitSiteState<int>>>(v1, ctr_from_range);
    const auto k2 = std::make_shared<kstate::DynamicKstate<kstate_trait::TraitSiteState<int>>>(v2, ctr_from_range);
    basis.add_element(k1);
...
TEST(Basis, BigTest) {
    ...
    const auto k0 = std::make_shared<kstate::DynamicKstate<kstate_trait::TraitSiteState<int>>>(v0, ctr_from_range);
    const auto k1 = std::make_shared<kstate::DynamicKstate<kstate_trait::TraitSiteState<int>>>(v1, ctr_from_range);
    const auto k2 = std::make_shared<kstate::DynamicKstate<kstate_trait::TraitSiteState<int>>>(v2, ctr_from_range);
    const auto k3 = std::make_shared<kstate::DynamicKstate<kstate_trait::TraitSiteState<int>>>(v3, ctr_from_range);
    const auto k4 = std::make_shared<kstate::DynamicKstate<kstate_trait::TraitSiteState<int>>>(v4, ctr_from_range);
    const auto k5 = std::make_shared<kstate::DynamicKstate<kstate_trait::TraitSiteState<int>>>(v5, ctr_from_range);
    const auto k6 = std::make_shared<kstate::DynamicKstate<kstate_trait::TraitSiteState<int>>>(v6, ctr_from_range);
    const auto k7 = std::make_shared<kstate::DynamicKstate<kstate_trait::TraitSiteState<int>>>(v7, ctr_from_range);
    const auto k8 = std::make_shared<kstate::DynamicKstate<kstate_trait::TraitSiteState<int>>>(v8, ctr_from_range);
    const auto k9 = std::make_shared<kstate::DynamicKstate<kstate_trait::TraitSiteState<int>>>(v9, ctr_from_range);
    const auto k10 = std::make_shared<kstate::DynamicKstate<kstate_trait::TraitSiteState<int>>>(v10, ctr_from_range);
    const auto k11 = std::make_shared<kstate::DynamicKstate<kstate_trait::TraitSiteState<int>>>(v11, ctr_from_range);
...
TEST(Basis, BigTestWithUniqueStates) {
    ...
    const int v8[3] = {3, 20, 15};
    const auto k0 = std::make_shared<kstate::DynamicKstate<kstate_trait::TraitSiteState<int>>>(kstate::make_unique_shift(v0), ctr_from_range);
    const auto k1 = std::make_shared<kstate::DynamicKstate<kstate_trait::TraitSiteState<int>>>(kstate::make_unique_shift(v1), ctr_from_range);
    const auto k2 = std::make_shared<kstate::DynamicKstate<kstate_trait::TraitSiteState<int>>>(kstate::make_unique_shift(v2), ctr_from_range);
    const auto k3 = std::make_shared<kstate::DynamicKstate<kstate_trait::TraitSiteState<int>>>(kstate::make_unique_shift(v3), ctr_from_range);
    const auto k4 = std::make_shared<kstate::DynamicKstate<kstate_trait::TraitSiteState<int>>>(kstate::make_unique_shift(v4), ctr_from_range);
    const auto k5 = std::make_shared<kstate::DynamicKstate<kstate_trait::TraitSiteState<int>>>(kstate::make_unique_shift(v5), ctr_from_range);
    const auto k6 = std::make_shared<kstate::DynamicKstate<kstate_trait::TraitSiteState<int>>>(kstate::make_unique_shift(v6), ctr_from_range);
    const auto k7 = std::make_shared<kstate::DynamicKstate<kstate_trait::TraitSiteState<int>>>(kstate::make_unique_shift(v7), ctr_from_range);
    const auto k8 = std::make_shared<kstate::DynamicKstate<kstate_trait::TraitSiteState<int>>>(kstate::make_unique_shift(v8), ctr_from_range);
    basis.add_element(k0);
    basis.add_element(k1);
    basis.add_element(k2);
    basis.add_element(k3);
    ...
    // test not-finding a not existing element:
    const int v100[3] = {11, 12, 113};
    const auto k100 = std::make_shared<kstate::DynamicKstate<kstate_trait::TraitSiteState<int>>>(kstate::make_unique_shift(v100), ctr_from_range);
    EXPECT_FALSE(basis.find_element_and_get_its_ra_index(k100->to_range()));
    EXPECT_FALSE(basis.find_element_and_get_its_ra_index(v100));
