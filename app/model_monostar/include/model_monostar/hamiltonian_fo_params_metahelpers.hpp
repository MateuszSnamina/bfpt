 ...
 *         = -2C * sin(θ) * [-A' + cos(θ)]
 *   d²f/dθ² = -2C * cos(θ) * [-A' + cos(θ)] + 2C * sin²(θ)
 *
 * Case when C ≠ 0 ∧ A' < -1:
 *   θ₁ = 0
 *   θ₂ = π
 ...
 *   θ₂ = π
 *   d²f/dθ²[θ₁] = -2C * [-A' + 1] ⇒ sgn(d²f/dθ²[θ₁]) = -sgn(C)sgn(1-A') = -sgn(C)
 *   d²f/dθ²[θ₂] = +2C * [-A' - 1] ⇒ sgn(d²f/dθ²[θ₁]) = -sgn(C)sgn(1+A') = 0
 *   θ₂ is a minimum if C > 0.
 *   θ₁ is a minimum if C < 0.
 *
 * Case when C ≠ 0 ∧ -1 < A' < +1:
 ...
 *   Note that: f(θ₁)= +A + C + Z
 *   Note that: f(θ₂)= -A + C + Z
 *   Note that: f(θ₃)= A*A' + C*A'² + Z
 *   Note that: f(θ₄)= A*A' + C*A'² + Z
 *   ⇒ θ₃ and θ₄ are global minima if C > 0.
 *   ⇒ θ₁ is global minimum if C < 0 and A < 0.
 *   ⇒ θ₂ is global minimum if C < 0 and A > 0.
 *   ⇒ θ₁ and θ₂ are global minima if C < 0 and A=0.
 *
 * Case when C ≠ 0 ∧ A' = +1:
 *   θ₁ = 0
 *   θ₂ = π
