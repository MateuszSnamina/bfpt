...
calculate_reduced_density_operator_12(
    ...
    for (arma::uword bra_kstate_idx = 0; bra_kstate_idx < eigen_vector.n_rows; bra_kstate_idx++) {
        for (arma::uword ket_kstate_idx = 0; ket_kstate_idx < eigen_vector.n_rows; ket_kstate_idx++) {
            ...
            const auto bra_kstate_range_rest = bra_kstate_range | boost::adaptors::sliced(2, n_sites);
            const auto ket_kstate_range_rest = ket_kstate_range | boost::adaptors::sliced(2, n_sites);
            if (boost::equal(bra_kstate_range_rest, ket_kstate_range_rest)) {
                const auto bra_site_0 = *std::next(std::begin(bra_kstate_range), 0);
                const auto bra_site_1 = *std::next(std::begin(bra_kstate_range), 1);
                const auto ket_site_0 = *std::next(std::begin(ket_kstate_range), 0);
                const auto ket_site_1 = *std::next(std::begin(ket_kstate_range), 1);
