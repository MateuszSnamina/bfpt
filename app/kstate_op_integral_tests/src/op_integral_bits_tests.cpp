...
TEST(KstateOpBits, Refined) {
    ASSERT_EQ(kstate_op_integral::refine<uint64_t>(0b1100101110, false, 3), 0b1100100110);
    ASSERT_EQ(kstate_op_integral::refine<uint64_t>(0b1100101110, true, 3), 0b1100101110);
    ASSERT_EQ(kstate_op_integral::refine<uint64_t>(0b1100101110, false, 4), 0b1100101110);
    ASSERT_EQ(kstate_op_integral::refine<uint64_t>(0b1100101110, true, 4), 0b1100111110);
}

TEST(KstateOpBits, ToBitsRange) {
    const auto r = kstate_op_integral::integral_to_bits_range<uint64_t>(0b1100101110, 10);
...
TEST(KstateOpBits, FromBitsRange) {
    ...
    const auto n = kstate_op_integral::integral_from_bits_range<uint64_t>(a);
